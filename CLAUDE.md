...
- **Weekly (週報)**: a calendar week = **Mon–Fri**. Two ways it can land in `site/data/weekly/<wkey>/`:
  - `build_weekly_from_daily.py <wkey>` (**preferred**) aggregates the existing daily `site/data/<date>/{code}.json` 分點 across the Mon–Fri window into `{code}.json` (買賣超前20) + `{code}_vol.json` (大量與均價: 大量價=當日買進量最大價位; top10 量=日 buy_top/sell_top 前10 加總; top10 均價=前10均價的**未加權平均**; 收盤價取自 `tw_volume.db`). Validate with `--check <existing-wkey>`. This avoids the wrong-week-boundary bug (don't use a trailing-5-trading-day window like `20260529-0604`; use Mon–Fri like `20260601-0605`).
